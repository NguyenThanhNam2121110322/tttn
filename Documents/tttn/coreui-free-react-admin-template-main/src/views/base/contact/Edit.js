...
import {     getContactsById,
  editContacts} from "../../../api/ContatcService";
...
const navigate = useNavigate();
  useEffect(() => {
    const fetchData = async () => {
      try {
        const contact = await getContactsById("Contact", idContact);
    ...
    const handleEditContact = async (event) => {
      ...
      if ( name !== "" ) {
        ...
        try {
          const editedContact = await editContacts(
            `Contact/${idContact}`,
            contact
          );
          setCheckUpdate(true);
