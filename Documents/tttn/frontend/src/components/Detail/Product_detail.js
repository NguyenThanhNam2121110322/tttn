  ...
  useEffect(() => {
    axios.get(baseURL + `products/${id}`)
      .then(response => {
        ...
        const categoryNames = response.data.categories.map(category => category.categoryName);
        // Lấy danh sách categoryName từ mỗi đối tượng Category trong Set<Category>
  
        const categoryAPI = `products/category/${categoryNames.join(",")}`;
        // Gắn categoryName vào link API, sử dụng phần tử categoryName ngăn cách nhau bằng dấu phẩy
  
        axios.get(baseURL + categoryAPI)
          .then(response => {
